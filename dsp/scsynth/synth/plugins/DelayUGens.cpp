...
// TAU WASM FIX: Removed unused 'int inNumSamples' parameter to match UnitCtorFunc signature.
...
// TAU WASM FIX: Removed unused 'int inNumSamples' parameter from all Info UGen constructors.
...
void RadiansPerSample_Ctor(Unit* unit) { ZOUT0(0) = unit->mParent->mFullRate->mRadiansPerSample; }
...
// TAU WASM FIX: Removed unused 'int inNumSamples' parameter.
// Original SuperCollider has this parameter but never uses it. In native builds, the unused
// parameter gets garbage from a register but causes no harm. In WebAssembly, the signature

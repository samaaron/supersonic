...
class ScsynthProcessor extends AudioWorkletProcessor {
    ...
    loadBufferConstants() {
        ...
        this.bufferConstants = {
            ...
            NRT_OUT_BUFFER_SIZE: uint32View[5],   // capacity for the nrt-out buffer-fill metric
    ...
    readNodeTree() {
        ...
        for (let i = 0; i < maxNodes && foundCount < nodeCount; i++) {
            ...
            const uuidRaw = new Uint8Array(this.wasmMemory.buffer, uuidStart, 16);
            let hasUuid = false;
            for (let j = 0; j < 16; j++) {
    ...
    process(inputs, outputs, parameters) {
        ...
        try {
            if (this.wasmInstance && this.wasmInstance.exports.ss_tick) {
                ...
                // PostMessage mode: read OSC replies and node tree updates
                if (this.mode === 'postMessage') {
                    this.readOscReplies();

...
// `/midi/clock/beat <port> <durMs>` — schedule one beat's worth (24 ticks) of
// MIDI clock spread over durMs, generated by the SuperClock-timed MidiClockOut
// (Sonic Pi's midi_clock_beat). /midi/clock/tick (one immediate pulse — also how
// MidiClockOut's scheduled burst ticks come back through the dispatch path) and
// /midi/clock/sync (clock-in tempo source) are not handled here and fall through
// to the Rust subsystem.
bool MidiControl::handleClockOutVerb(const uint8_t* data, uint32_t size) {
    if (!mClock || size < 16) return false;
    const char* addr = reinterpret_cast<const char*>(data);
    if (std::strcmp(addr, "/midi/clock/beat") != 0) return false;
    ...
    try {
        ...
        if (it != msg.ArgumentsEnd()) {
            if (it->IsFloat())       num = it->AsFloatUnchecked();
            else if (it->IsDouble()) num = it->AsDoubleUnchecked();
            else if (it->IsInt32())  num = it->AsInt32Unchecked();
        }
    } catch (...) { return true; }  // malformed beat verb — swallow
    get_midi_clock_out().onBeat(*mClock, port, num / 1000.0);
    return true;
}

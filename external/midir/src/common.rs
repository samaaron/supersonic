...
impl MidiOutputConnection {
    ...
    /// Schedule a message for future delivery at `timestamp_ms`, a
    /// `DOMHighResTimeStamp` (same coordinate as `performance.now()`). The
    /// browser's MIDI subsystem performs the precise scheduling — the basis for
    /// a rock-solid, look-ahead MIDI clock on the web. Web MIDI backend only.
    #[cfg(target_arch = "wasm32")]
    pub fn send_at(&mut self, message: &[u8], timestamp_ms: f64) -> Result<(), SendError> {
        self.imp.send_at(message, timestamp_ms)
    }
}

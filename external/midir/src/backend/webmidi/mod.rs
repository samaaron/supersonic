...
impl MidiOutputConnection {
    ...
    /// Schedule `message` for delivery at `timestamp_ms`, a `DOMHighResTimeStamp`
    /// in the same time coordinate as `performance.now()`. The browser's MIDI
    /// subsystem performs the precise scheduling, which makes this the basis for
    /// a rock-solid MIDI clock on the web (look-ahead scheduling). A timestamp in
    /// the past sends immediately, matching the Web MIDI spec.
    pub fn send_at(&mut self, message: &[u8], timestamp_ms: f64) -> Result<(), SendError> {
        self.output
            .send_with_timestamp(unsafe { Uint8Array::view(message) }.as_ref(), timestamp_ms)
            .map_err(|_| SendError::Other("JavaScript exception"))
    }
}

...
test.describe("getTree() UUID support", () => {
  test("UUID-created synth has uuid field in getRawTree", async ({ page, sonicConfig }) => {
    await page.goto("/test/harness.html");
    ...
    const result = await page.evaluate(async (config) => {
      ...
      const tree = sonic.getRawTree();
      // Find the synth node (not the root group or default group)
      const synth = tree.nodes.find((n) => !n.isGroup && n.defName === "sonic-pi-beep");
      await sonic.shutdown();

      return {
        found: !!synth,
        hasUuid: synth?.uuid !== null && synth?.uuid !== undefined,
        uuidBytes: synth?.uuid ? Array.from(synth.uuid) : null,
      };
    }, sonicConfig);

    expect(result.found).toBe(true);
    expect(result.hasUuid).toBe(true);
    expect(result.uuidBytes).toEqual([
      0x01, 0x93, 0xa5, 0xb0, 0x7c, 0x8a, 0x70, 0x00,
      0x80, 0x00, 0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe,
    ]);
  });

  test("int32-created synth has null uuid in getRawTree", async ({ page, sonicConfig }) => {
    await page.goto("/test/harness.html");
    ...
    const result = await page.evaluate(async (config) => {
      ...
      await sonic.send("/s_new", "sonic-pi-beep", id, 0, 0, "release", 60);
      await sonic.sync(1);
      await new Promise((r) => setTimeout(r, 100));

      const tree = sonic.getRawTree();
      const synth = tree.nodes.find((n) => n.id === id);
      await sonic.shutdown();

      return {
        found: !!synth,
        uuidIsNull: synth?.uuid === null,
      };
    }, sonicConfig);

    expect(result.found).toBe(true);
    expect(result.uuidIsNull).toBe(true);
  });

  test("getTree() surfaces UUID as node id for UUID-created synths", async ({ page, sonicConfig }) => {
    await page.goto("/test/harness.html");
    ...
    const result = await page.evaluate(async (config) => {
      ...
      // Find the synth in the hierarchical tree
      const findSynth = (node) => {
        if (node.type === "synth" && node.defName === "sonic-pi-beep") return node;
        for (const child of node.children || []) {
          const found = findSynth(child);
          if (found) return found;
        }
        return null;
      };
      const synth = findSynth(tree.root);
      await sonic.shutdown();

      return {
        found: !!synth,
        idIsUuid: synth?.id instanceof Uint8Array,
        idBytes: synth?.id instanceof Uint8Array ? Array.from(synth.id) : null,
      };
    }, sonicConfig);

    expect(result.found).toBe(true);
    expect(result.idIsUuid).toBe(true);
    expect(result.idBytes).toEqual([
      0x01, 0x93, 0xa5, 0xb0, 0x7c, 0x8a, 0x70, 0x00,
      0x80, 0x00, 0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe,
    ]);
  });

  test("UUID is cleared from tree after synth is freed", async ({ page, sonicConfig }) => {
    await page.goto("/test/harness.html");
    ...
    const result = await page.evaluate(async (config) => {
      ...
      // Verify it's there
      const treeBefore = sonic.getRawTree();
      const synthBefore = treeBefore.nodes.find((n) => n.defName === "sonic-pi-beep");
      const hadUuid = synthBefore?.uuid !== null;
      ...
      // Verify it's gone
      const treeAfter = sonic.getRawTree();
      const synthAfter = treeAfter.nodes.find((n) => n.defName === "sonic-pi-beep");

      await sonic.shutdown();

      return {
        hadUuid,
        synthGone: !synthAfter,
      };
    }, sonicConfig);

    expect(result.hadUuid).toBe(true);
    expect(result.synthGone).toBe(true);
  });
});

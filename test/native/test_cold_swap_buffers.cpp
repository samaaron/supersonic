/*
 * test_cold_swap_buffers.cpp — buffers across cold swaps
 *
 * Every case allocates with /b_alloc, which is the engine's own heap: a
 * buffer's memory belongs to the World that made it, and a cold swap replaces
 * the World. Validates that:
 *   - a buffer allocated before a swap does not crash the swap
 *   - allocation works again after a swap, on the new World
 *   - repeated swaps with buffer activity leave the heap intact
 *   - a synthdef reloads after a swap
 */
...
static bool synthdefsAvailable() {
    std::string path = std::string(CLOCKWORK_SYNTHDEFS_DIR) + "/sonic-pi-beep.scsyndef";
    return std::filesystem::exists(path);
}

// ── Buffer load survives cold swap ──────────────────────────────────────────

TEST_CASE("ColdSwap: buffer load before swap doesn't crash", "[ColdSwap][Buffers]") {
    EngineFixture fix;
    ...
    // Allocate a buffer on the engine's own heap
...
TEST_CASE("ColdSwap: buffer allocation works after swap", "[ColdSwap][Buffers]") {
    EngineFixture fix;
...
TEST_CASE("ColdSwap: multiple swaps with buffers don't corrupt heap", "[ColdSwap][Buffers]") {
    EngineFixture fix;
...
TEST_CASE("ColdSwap: synthdef reload works after swap", "[ColdSwap][Buffers]") {

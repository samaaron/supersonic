 ...
 * without cross-compiling. The active build's derived macros (CLOCKWORK_LEAN_TARGET,
 * CLOCKWORK_COLD_BSS) are checked directly, since the host compiles the desktop branch.
 */
...
// Hardware double FPU: desktop + WASM yes (clockwork_calc_t == double, byte-identical
// upstream DSP); ESP32 LX7 no (clockwork_calc_t degrades to float). Freestanding mirrors
...
#endif
CLOCKWORK_COLD_BSS static int supersonic_cold_bss_probe = 0;  // empty expansion must compile on host
...
TEST_CASE("SC_Platform exposes a coherent capability matrix", "[platform]") {
    ...
    (void)supersonic_cold_bss_probe;
}

 ...
 *
 * The host build is desktop (clockwork_calc_t == double), so on its own it never
 * compiles the float branch of clockwork_calc_t / clockwork_calc_guard. SC_CalcType.inc is
...
#include "platform.h" // host build: clockwork_calc_t = double, clockwork_calc_guard = identity

// The host (desktop) build must be full double precision with the guard a pure
// pass-through, i.e. hosted DSP behaviour unchanged by the clockwork_calc_t conversion.
...
TEST_CASE("clockwork_calc_guard: host double path is a pass-through", "[clockwork_calc]") {
...
TEST_CASE("clockwork_calc_guard: float path resets non-finite / runaway state", "[clockwork_calc]") {
...
TEST_CASE("float biquad: clockwork_calc_guard bounds a diverging recursion", "[clockwork_calc]") {
    using F = flt::clockwork_calc_t;
